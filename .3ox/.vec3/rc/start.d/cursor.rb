#!/usr/bin/env ruby
# start.d/cursor.rb - Cursor startup flow for `3ox start cursor`.
...
CURSOR_RULES_PATH = File.join(WORKSPACE_ROOT, '.cursorrules')
...
ensure_readable!(CURSOR_RULES_PATH)
ensure_readable!(SPARKFILE_PATH)
ensure_readable!(STARTUP_SCRIPT_PATH)

ok = system('ruby', STARTUP_SCRIPT_PATH, 'cursor')
exit(1) unless ok
