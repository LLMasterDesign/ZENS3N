...
- Last verification (UTC): 2026-03-04 15:06:36Z
...
- HTTPS API telemetry (via nginx) is reachable even though SSH auth is blocked:
  ...
  - Command-endpoint method probe results (via HTTPS front door):
    ...
    - latest `/health` response timestamp now reads `2026-03-04T15:06:36.891301Z` with services still `pulse=true`, `tape=true`, `warden=true`.
