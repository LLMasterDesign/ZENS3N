...
- Last verification (UTC): 2026-03-04 14:54:08Z
...
- HTTPS API telemetry (via nginx) is reachable even though SSH auth is blocked:
  ...
  - direct HTTPS probes reconfirm cursor routes are not exposed on nginx front door:
    ...
    - hostname probes show identical behavior:
      ...
      - health parity recheck: `GET https://1n3ox.ai/health` returned the same healthy service booleans as IP-based `/health` (`pulse=true`, `tape=true`, `warden=true`).
  ...
  - Command-endpoint method probe results (via HTTPS front door):
    ...
    - latest `/health` response timestamp now reads `2026-03-04T14:54:08.190821Z` with services still `pulse=true`, `tape=true`, `warden=true`.
