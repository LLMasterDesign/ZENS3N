...
- Last verification (UTC): 2026-03-04 10:39:20Z
...
- HTTPS API telemetry (via nginx) is reachable even though SSH auth is blocked:
  ...
  - Command-endpoint method probe results (via HTTPS front door):
    ...
    - latest `/health` response timestamp now reads `2026-03-04T10:39:20.281474Z` (hostname probe `10:39:20.489713Z`) with services still `pulse=true`, `tape=true`, `warden=true`.
