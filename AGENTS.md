...
- **Rust toolchain**: Because there is no `Cargo.lock`, `cargo` resolves transitive deps (e.g. `hashbrown`) to versions requiring `edition2024` / Rust >= 1.85. The base image's Rust 1.83 fails to build with a "feature `edition2024` is required" error — run `rustup update stable && rustup default stable` (the update script handles this) to get a current stable toolchain before building.
