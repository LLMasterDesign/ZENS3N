...
def log_operation(operation, status, details = "")
  ...
  # Auto-increment discovery turn counter
  if status == "COMPLETE"
    discovery_script = "R:/!LAUNCH.PAD/!CMD.CENTER/Toolkit/chat_discovery.rb"
    observation = "#{operation}: #{details[0..80]}"
    system("ruby #{discovery_script} increment \"#{observation}\" 2>&1 > NUL")
  end
end

def find_or_create_output_folder
  """7HE.CITADEL outputs ONLY to !CITADEL.OPS/!0UT.3OX CITADEL (ENFORCED)"""
  
  # CITADEL has fixed output location - no dynamic discovery needed
  output_folder = "R:/!LAUNCH.PAD/!CITADEL.OPS/!0UT.3OX CITADEL"
  
  # Ensure it exists
  FileUtils.mkdir_p(output_folder) unless File.exist?(output_folder)
  
  # Ensure receipts subfolder exists
  receipts_dir = File.join(output_folder, "receipts")
  FileUtils.mkdir_p(receipts_dir) unless File.exist?(receipts_dir)
